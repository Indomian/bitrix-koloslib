...
if(class_exists("kolos_dummy")) return;//Защита от повторного подключения

Class kolos_dummy extends CModule {
    ...
    function kolos_dummy()
    {
        $this->MODULE_VERSION = GetMessage("KOLOS_DUMMY_VERSION");//Устанавливаем версию модуля...
        $this->MODULE_VERSION_DATE = GetMessage("KOLOS_DUMMY_VERSION_DATE");//...и дату выпуска
        $this->MODULE_NAME = GetMessage("KOLOS_DUMMY_MODULE_NAME");//Название из языкового файла
        $this->MODULE_DESCRIPTION = GetMessage("KOLOS_DUMMY_MODULE_DESCRIPTION");
    ...
    function DoInstall()
    {
        ...
        $FORM_RIGHT = $APPLICATION->GetGroupRight($this->MODULE_ID);//Нелепая проверка, есть ли у пользователя доступ к модулю, который мы ещё не установили, помогает узнать админ ли 
        if ($FORM_RIGHT=="W")
        {
            $step = IntVal($step);//Продолжение бреда, убедимся, что число целое :)
            if($step<2)
            {
                $APPLICATION->IncludeAdminFile(GetMessage("KOLOS_DUMMY_INSTALL_TITLE"),
    ...
    function DoUninstall()
    {
        ...
        if ($FORM_RIGHT=="W")
        {
            $step = IntVal($step);
            if($step<2){
//                 $APPLICATION->IncludeAdminFile(GetMessage("KOLOS_DUMMY_UNINSTALL_TITLE"),
...
//             }
    ...
    function GetModuleRightList() //Метод возвращает права доступа к модулю, права обозначаются буквами, по приниципу A<Z, Т.е. права уровня A ниже чем права уровня Z, данные права используются при установке прав доступа для групп пользователей.
    {
        $arr = array(
            ...
            "reference" => array(
                GetMessage("KOLOS_DUMMY_DENIED"),
                GetMessage("KOLOS_DUMMY_OPENED"),
                GetMessage("KOLOS_DUMMY_FULL"))
            );
        return $arr;
    }
}
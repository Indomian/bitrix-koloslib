...
include_once KS_MODULES_DIR.'/koloslib/classes/general/interface.IKSListObject.php';
/**
 * Основной класс системы, обеспечивает выборку данных из БД, наследует от класса CKSBaseObject, реализует интерфейс IKSListObject
 * @author BlaDe39 <blade39@kolosstudio.ru>, Dmitry Konev <d.konev@kolosstudio.ru>
 * @version 1.0
 * @since 25.01.2012
 */
class CKSObject extends CKSBaseObject implements IKSListObject
{
	...
	/**
	 * Метод возвращает имя текущей таблицы объекта
	 */
	...
	/**
	 * Метод возвращает список полей объекта
	 */
	...
	/**
	 * Метод устанавливает ключевое поле, значение которого будет использоваться в качестве ключа массива результата
	 */
	public function SetKeyField($sField)
	{
		$this->sKeyField=$sField;
	}

	/**
	 * Метод возвращает текущее значение ключевого поля
	 */
	...
	/**
	 * Добавляет автоинкрементные поля
	 * @param string||array - поля(е) которые должны быть автоинкрементными
	 * @return void
	 */
	...
	protected function _SaveData($arData)
	{
		...
		if(array_key_exists('id',$arData) && $arData['id']!='')
		{
			...
			if ($this->obDB->num_rows()>0)
			{
				...
				$res = $arData['id'];
				if($res)
					unset(CKSObject::$arCache[$this->sTable][$res]);
				return $res;
			}
		}
		...
		if($fields=='' || $values=='') throw new CKSError('KOLOSLIB_INSERT_EMPTY',2001);
	...
	{
		if(!is_array(CKSObject::$arCache)) CKSObject::$arCache=array();
	 ...
	 * @param $id - номер записи возвращаемой по запросу
	 */
	function GetById($id)
	{
		if(is_array(CKSObject::$arCache))
			if(array_key_exists($this->sTable,CKSObject::$arCache))
				if(array_key_exists($id,CKSObject::$arCache[$this->sTable]))
					return CKSObject::$arCache[$this->sTable][$id];
	 ...
	 * @sa CKSObject::GetList(), CKSObject::_GenWhere(), CKSObject::_GenSelect()
	 ...
	 * @sa CKSObject::GetList(), CKSObject::_GenWhere()
	 * @since используется начиная с версии класса 2.1
	 *
	...
	{
		$arTableStructure=CKSObject::$dbStructure[$this->sTable];
		$arFields=array();
		foreach($arTableStructure as $sField=>$arField)
			if(isset($arField['Type']))
			{
				if(preg_match('#^enum#i',$arField['Type']))
	...
	/**
	 * Метод генерирует поиск по строке Having
	 */
	protected function _GenHaving($arFilter=false)
	{
		if(!is_array($arFilter)) return '';
		if(is_array($this->arFilter))
			$arFilter=array_merge($arFilter,$this->arFilter);
		...
		if(count($arFil)>0)
			return " HAVING ".join(" $method ",$arFil);
		return '';
	}
	
	/**
	 * Метод генерирует запрос по переданным параметрам. Полезно для использования при сборке сложных запросов
	 * @param array - сортировка
	 * @param array - фильтрация
	 * @param array||int - лими
	 * @param array - поля, которые надо выбирать
	 * @param array - поля, по которым будем группировать
	 * @param array - поля, по которым выбирать используя Having
	 * @return string
	 */
	function GetQuery($arOrder=false,$arFilter=false,$limit=false,$arSelect=false,$arGroupBy=false,$arHaving=false)
	{
		...
		/*Генерируем фильтр HAVING*/
		$sHaving=$this->_GenHaving($arHaving);
		...
		if(!$arSelect && (count($this->arJoinTables)>0 || count($this->arTables)>1))
			throw new CKSError('SYSTEM_QUERY_FIELDS_REQUIRE');
		...
		$query.=" $fields FROM $sFrom $sWhere $sGroupBy $sHaving $sOrder $limits";
		return $query;
	}
	...
	{
		$query=$this->GetQuery($arOrder,$arFilter,$limit,$arSelect,$arGroupBy);
		if(KS_DEBUG_QUERIES==1) echo $query.'<br/>';
		$ks_db_res=$this->obDB->query($query);
		if($this->obDB->num_rows($ks_db_res)<1)
			return false;
		$res=array();
		while ($item=$this->obDB->get_row($ks_db_res))
			if($this->_ParseItem($item))
			{
				if(array_key_exists($this->sKeyField,$item)&&$this->bJustAdd==false)
					$res[$item[$this->sKeyField]]=$item;
				else
					$res[]=$item;
			}
		$this->data=$res;
...
/**
	 ...
	 *
	 * @param array $arFilter ассоциативный массив филтрации, также смотрите CKSObject::_GenWhere.
	...
	function Update($id, $values,$bFiltered=false)
	{
		...
		if (strlen($sWhere)>0)
		{
			...
			//echo $query;
			$this->obDB->query($query);
			unset(CKSObject::$arCache[$this->sTable]);
			return $this->obDB->AffectedRows();
		}
		return -1;
	}
	 ...
	 * записей, является оберткой для метода {@link CKSObject::DeleteItems() CKSObject::DeleteItems()}
